...
export class RaceResultsToolComponent implements OnInit {
  ...
  dl() {
    var data = "data:text/json;charset=utf-8," + encodeURIComponent(this.output);
  ...
  run() {
    ...
    for (let i = 0; i < resultsJSON.sessionResult.leaderBoardLines.length; i++) {
      // console.log(i)
      ...
      reverseGrid - i > 0 ? position = reverseGrid - i : position = position;

      // console.log(`Car #${carNo} will start in P${position}\n`);

      for (let key in entrylistJSON.entries) {
        if (entrylistJSON.entries[key].raceNumber == carNo) entrylistJSON.entries[key].defaultGridPosition = position
      }
