...
export class EntrylistEditorComponent implements OnInit {
  ...
  patchForm(key: number) {
    this.form.patchValue({
      firstName: this.json.entries[key].drivers[0].firstName,
      lastName: this.json.entries[key].drivers[0].lastName,
      nickname: this.json.entries[key].drivers[0].nickname,
      shortName: this.json.entries[key].drivers[0].shortName,
      driverCategory: this.json.entries[key].drivers[0].driverCategory,
      steamId: this.json.entries[key].drivers[0].playerID.substring(1),
      customCarName: this.json.entries[key].customCar,
      raceNumber: this.json.entries[key].raceNumber,
      nationality: this.json.entries[key].drivers[0].nationality,
      carChoice: this.json.entries[key].forcedCarModel,
  ...
  getDriverOrders() {
    ...
    try {
      this.orderedDrivers = tempJSON.entries.map((entry, index) => {
        return {
  ...
  updateGridPosition() {
    for(let i = 0; i < this.orderedDrivers.length; i++) {
      this.json.entries[this.orderedDrivers[i]]
      .defaultGridPosition = i + 1;
    }

    for(let i = 0; i < this.unorderedDrivers.length; i++) {
      this.json.entries[this.unorderedDrivers[i]]
  ...
  saveData() {
    ...
    this.json.entries[this.driverIndex].drivers[0].shortName = this.form.get("shortName").value;
    this.json.entries[this.driverIndex].drivers[0].driverCategory = this.form.get("driverCategory").value;
    this.json.entries[this.driverIndex].drivers[0].playerID = "S" + this.form.get("steamId").value;
    this.json.entries[this.driverIndex].customCar = this.form.get("customCarName").value;
